    ...
    if(isset($_POST['rmcode']) && isset($_POST['rmdate'])&& isset($_POST['rmcategory'])&& isset($_POST['rmdisc'])&& isset($_POST['rmqty'])&& isset($_POST['rmrate'])&& isset($_POST['rmsupplier'])&& isset($_POST['rmunit']))
{
    ...
    $insertstocksql="INSERT INTO `reciptmaster`(`reciptdate`, `itemcode`, `category`, `reciptdisc`, `qty`, `rate`, `supplier`,`rmunitcode`) 
            ...
            {			
            $output="Done";
			$insertedreciptid = mysqli_insert_id($con);
			getreciptdatafortransaction($con,$insertedreciptid);
...
if(isset($_POST['currentstock']) && isset($_POST['uprmcode'])&& isset($_POST['newstock'])&& isset($_POST['narration']))
{
	$updateqty="UPDATE `stocktable` SET `qty`=$newstock WHERE `itemcode`='$uprmcode'";
    mysqli_query($con,$updateqty);
	$today=date("Y-m-d");
	makestocklogentry($con,$uprmcode,$currentstock,$newstock,$narration,$today);
