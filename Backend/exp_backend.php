...
// include('functions.php');
    ...
    if(isset($_POST['ename']) && isset($_POST['eamt']) && isset($_POST['slspaytype']) )
{
    // echo "Demo ";
    $inseruser="INSERT INTO `expencemaster`(`ename`, `eamt`, `etype`, `status`, `disc`, `createdby`)
                                    VALUES ('$ename','$eamt','$slspaytype','OPEN','','$loginuserid')";
            // echo $inseruser;
    ...
    if(isset($_POST['txtid']) && isset($_POST['pfees'])&& isset($_POST['slspaytype']))
{
    ...
    $inseruser="UPDATE `opdmaster` SET `opdstatus`='DONE' ,`opdfair`='$pfees',`paymenttype`='$slspaytype' WHERE id='$txtid'";
		...
		if(isset($_POST['deleteid']))
		{
		  ...
		  $sql="UPDATE `expencemaster` SET `status`='CLOSED' WHERE id='$deleteid'";
		  //echo($sql);
