    ...
    {?>
        <tr>
          ...
          <td><?php echo $row['deliverydate']; ?></td>
            ...
            <?php
            if(isitemcodeexist($con,$row['itemcode'])=="true")
            { ?>
            ...
            <td id="txtitemnotexist">
             <a href="manageitem.php?itemcode=<?php echo $row['itemcode'] ?>" class="text-danger"> <?php echo $row['itemcode']; ?></a>
            </td>
          ...
          if($row['isdeleted']==1)
          {?>
        <button class="btn btn-primary" onclick=softRecoverthispo(<?php echo $row['id']; ?>)>Recover</button>
          <?php
          }else
          {?>
            <button class="btn btn-danger" onclick=softdeletethispo(<?php echo $row['id']; ?>)>X</button>
          <?php
          }
...
?>
  ...
  $(document).ready( function () {
    ...
    function softdeletethispo(poid)
    {
      // alert(poid);
      swal({
    title: "Are you sure?",
    text: "Once deleted, you will not be able to recover this PO line !",
    icon: "warning",
    buttons: true,
    dangerMode: true,
  })
  .then((willDelete) => {
    ...
    function softRecoverthispo(poidforrecover)
    {
      // alert(poidforrecover);

      $.ajax({
                    url: "po_backend.php",
                    type: "POST",
                    data: {poidforrecover:poidforrecover},
                    success:function(data) {
                      // swal("Poof! Your imaginary file has been Recovered!", {
                      //   icon: "success",
                      // });
                        location.reload(true);
                    },
  ...
  .then((willDelete) => {
    ...
    // function senddatatocopyfunction()
    // {
    //   var textToCopy = $('#txtitemnotexist').text();
    //   copyTextToClipboard(textToCopy)
    // }

    // function copyTextToClipboard(itemcode) {
    //   var tempInput = $('<input>');
    //   $('body').append(tempInput);
    //   tempInput.val(itemcode).select();
    //   document.execCommand('copy');
    //   tempInput.remove();
    //   console.log('Text copied:', itemcode);
    // }
  </script>